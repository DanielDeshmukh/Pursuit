...
export default function BadgeModal({ open, onClose, data, onSave }: BadgeModalProps) {
  ...
  const handlePhotoSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async () => {
      ...
      try {
        ...
        setProcessError(
          err instanceof Error ? err.message : "Processing failed — using original photo"
        );
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      const orig: Partial<BadgeData> = data || {};
      ...
      if (Number(form.overall) !== (orig.overall ?? 0))
        payload.overall = Number(form.overall) ?? null;
  ...
  const inputClass =
    "w-full rounded-lg border border-hairline bg-cloud px-3 py-2 text-sm text-ink outline-none focus:border-primary focus:ring-1 focus:ring-primary";
      ...
      >
        <div className="mb-5 flex items-center justify-between">
          ...
          <button
            ...
            className="rounded-lg p-1.5 text-graphite hover:bg-cloud hover:text-ink"
          >
            <svg
              width="20"
              height="20"
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            {processing === "processing" && <StepIndicator current={processing} />}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input
                ...
                max="99"
                value={form.overall}
                onChange={(e) => setForm({ ...form, overall: Number(e.target.value) })}
              />
            </div>
            <div>
          ...
          <div className="border-t border-hairline pt-4">
            <p className="mb-3 text-xs font-bold uppercase tracking-wider text-graphite">
              Stats (0-99)
            </p>
            <div className="grid grid-cols-3 gap-3">
              <div>
                ...
                <input
                  ...
                  max="99"
                  value={form.proj}
                  onChange={(e) => setForm({ ...form, proj: Number(e.target.value) })}
                />
              </div>
              <div>
                ...
                <input
                  ...
                  max="99"
                  value={form.tech}
                  onChange={(e) => setForm({ ...form, tech: Number(e.target.value) })}
                />
              </div>
              <div>
                ...
                <input
                  ...
                  max="99"
                  value={form.cont}
                  onChange={(e) => setForm({ ...form, cont: Number(e.target.value) })}
                />
              </div>
              <div>
                ...
                <input
                  ...
                  max="99"
                  value={form.yexp}
                  onChange={(e) => setForm({ ...form, yexp: Number(e.target.value) })}
                />
              </div>
              <div>
                ...
                <input
                  ...
                  max="99"
                  value={form.cert}
                  onChange={(e) => setForm({ ...form, cert: Number(e.target.value) })}
                />
              </div>
              <div>
                ...
                <input
                  ...
                  max="99"
                  value={form.lang}
                  onChange={(e) => setForm({ ...form, lang: Number(e.target.value) })}
                />
              </div>
            </div>
          </div>
        ...
        <div className="mt-6 flex justify-end gap-3">
          <button
            ...
            className="rounded-lg px-4 py-2 text-sm font-medium text-graphite hover:bg-cloud"
          >
            Cancel
          </button>
          <button
