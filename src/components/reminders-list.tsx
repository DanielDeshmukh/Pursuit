...
export function RemindersList() {
  ...
  async function handleAddToCalendar(r: ReminderWithApp) {
    try {
      const res = await fetch("/api/calendar/ics", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          title: `${r.type.replace("_", " ")} - ${r.jobTitle} @ ${r.companyName}`,
          description: `Reminder for ${r.jobTitle} at ${r.companyName}`,
          startDate: r.dueAt,
        }),
      });
      ...
      a.href = url;
      a.download = `${r.type}-${r.companyName}.ics`;
      a.click();
