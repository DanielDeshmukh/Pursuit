...
export default function ProfilePage() {
  ...
  function addWorkBullet(workIdx: number) {
    setWorkEntries((w) =>
      w.map((e, i) => (i === workIdx ? { ...e, bullets: [...e.bullets, ""] } : e))
    );
  }
  ...
  function addProjectBullet(projIdx: number) {
    setProjectEntries((p) =>
      p.map((e, i) => (i === projIdx ? { ...e, bullets: [...e.bullets, ""] } : e))
    );
  }
  ...
  async function handleResumeUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const hasExisting =
        EDITABLE_KEYS.some((k) => original[k]?.trim()) ||
  ...
  function applyParsedData(parsed: Profile, merge: boolean) {
    ...
    if (merge) {
      ...
      if (projectEntries.length === 0 && parsedProjects.length > 0)
    ...
    <SidebarLayout>
          ...
          {showOverwriteWarning && parseResult && (
            <div className="rounded-lg border border-amber-800/50 bg-amber-950/30 p-5 space-y-4">
              <div className="flex items-start gap-3">
                <div>
                  ...
                  <p className="mt-1 text-xs text-amber-300/70">
                    Choose how to apply the parsed resume data.
                  </p>
                </div>
              </div>
                    ...
                    {EDITABLE_KEYS.filter((k) => parseResult[k]?.trim()).map((k) => {
                      ...
                      return (
                        <tr key={k} className="border-b border-zinc-800">
                          ...
                          <td
                            className={`px-3 py-2 ${d.changed ? "font-medium text-amber-300" : "text-zinc-400"}`}
                          >
                            {d.new}
                          </td>
                        </tr>
              ...
              <div className="flex items-center gap-3">
                <button
                  onClick={() => applyParsedData(parseResult, false)}
                  className="rounded-md bg-white px-4 py-2 text-xs font-medium text-black hover:bg-zinc-200 transition"
                >
                  Overwrite All
                </button>
                <button
                  onClick={() => applyParsedData(parseResult, true)}
                  className="rounded-md border border-zinc-700 px-4 py-2 text-xs font-medium text-zinc-300 hover:bg-zinc-800 transition"
                >
                  Merge
                </button>
                <button
              ...
              <section className="flex flex-col gap-6 lg:flex-row">
                ...
                <div className="flex-1 min-w-0 space-y-3">
                  <div>
                    ...
                    {(form.currentTitle || form.currentCompany) && (
                      <p className="mt-1 text-sm text-zinc-400">
                        {form.currentTitle}
                        {form.currentTitle && form.currentCompany ? " at " : ""}
                        {form.currentCompany}
                      </p>
                    )}
                  </div>
                  ...
                  <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-zinc-500">
                    ...
                    {form.yearsExperience && <span>{form.yearsExperience} years experience</span>}
                  </div>
                        ...
                        <div className="flex items-baseline justify-between gap-4">
                          ...
                          {p.bullets.filter(Boolean).length > 0 && (
                            <span className="shrink-0 text-[10px] text-zinc-500">
                              {p.bullets.filter(Boolean).length} bullet
                              {p.bullets.filter(Boolean).length !== 1 ? "s" : ""}
                            </span>
                          )}
                        </div>
                        {p.description && (
            ...
            <section className="rounded-lg border border-zinc-800 bg-zinc-900/50 p-12 text-center space-y-4">
              ...
              <div className="flex items-center justify-center gap-3">
                ...
                <button
                  ...
                  disabled={parsing}
                  className="rounded-md border border-zinc-700 px-4 py-2 text-xs font-medium text-zinc-300 hover:bg-zinc-800 disabled:opacity-50 transition"
                >
          ...
          {editing && (
            <>
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                ...
                <section className="rounded-xl border border-hairline bg-paper p-4">
                  ...
                  <p className="text-xs text-graphite mb-3">
                    Upload a PDF to auto-fill all fields below.
                  </p>
                  <input
                    ref={resumeInputRef}
                    type="file"
                  ...
                  <button
                    onClick={() => resumeInputRef.current?.click()}
                    disabled={parsing}
                    className="rounded-md border border-hairline bg-canvas px-4 py-2 text-xs font-medium text-ink hover:bg-cloud disabled:opacity-50 w-full"
                  >
              ...
              <section className="rounded-xl border border-hairline bg-paper p-5 space-y-4">
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                  {(["firstName", "lastName", "email", "phone", "city", "country"] as const).map(
                    (k) => (
                      <div key={k}>
              ...
              <section className="rounded-xl border border-hairline bg-paper p-5 space-y-4">
                ...
                <div>
                  ...
                  <p className="mt-1 text-[11px] text-graphite">
                    Max 100 words. Used by the Chrome extension for auto-fill.
                  </p>
                </div>
              </section>
              ...
              <section className="rounded-xl border border-hairline bg-paper p-5 space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <button
                    onClick={() => setWorkEntries((w) => [...w, emptyWork()])}
                ...
                {workEntries.length === 0 && (
                  <p className="text-xs text-graphite italic">
                    No entries yet. Add your work experience above.
                  </p>
                )}
                {workEntries.map((w, i) => (
                  <div
                    key={i}
                    className="rounded-lg border border-hairline bg-canvas p-4 space-y-3"
                  >
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-medium text-graphite">Entry {i + 1}</span>
                      <button
                        onClick={() => setWorkEntries((e) => e.filter((_, j) => j !== i))}
                    ...
                    <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                      <div>
                        <label className="mb-1 block text-[11px] font-medium text-graphite">
                          Company
                        </label>
                        <input
                          type="text"
                      ...
                      {w.bullets.map((b, j) => (
                        <div key={j} className="flex gap-2 mb-1.5">
                          <input
                            type="text"
                            value={b}
                            onChange={(e) => updateWorkBullet(i, j, e.target.value)}
                          ...
                          {w.bullets.length > 1 && (
                            <button
                              onClick={() => removeWorkBullet(i, j)}
                              className="text-graphite hover:text-error text-xs"
                            >
                              ✕
                            </button>
                          )}
              ...
              <section className="rounded-xl border border-hairline bg-paper p-5 space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <button
                    onClick={() => setProjectEntries((p) => [...p, emptyProject()])}
                    className="text-xs text-primary hover:underline font-medium"
                  >
                    + Add Entry
                  </button>
                ...
                {projectEntries.length === 0 && (
                  <p className="text-xs text-graphite italic">
                    No entries yet. Add your projects above.
                  </p>
                )}
                {projectEntries.map((p, i) => (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-medium text-graphite">Entry {i + 1}</span>
                      <button
                        onClick={() => setProjectEntries((e) => e.filter((_, j) => j !== i))}
                        className="text-xs text-error hover:underline"
                      >
                        Remove
                      </button>
                      ...
                      {p.bullets.map((b, j) => (
                        <div key={j} className="flex gap-2 mb-1.5">
                          <input
                            ...
                            placeholder="What was built or achieved..."
                            className="flex-1 rounded-md border border-steel bg-paper px-3 py-1.5 text-sm text-ink focus:border-ink focus:outline-none"
                          />
                          {p.bullets.length > 1 && (
                            <button
      ...
      {showBadgeView && (
        <div
          ...
          onClick={() => setShowBadgeView(false)}
        >
          <div
            className="relative bg-[#09090b] border border-zinc-800 rounded-xl p-5 w-[320px] mx-4 max-h-[90vh] flex flex-col"
